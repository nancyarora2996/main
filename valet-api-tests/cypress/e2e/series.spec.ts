...
describe('Valet API - Series Tests', () => {
    ...
    const testCurrencyPair = (seriesId: string, label: string) => {
      it(`should get the average rate for ${label} over the past 10 weeks`, () => {
        ...
        cy.request({
          ...
          const rates: number[] = observations.map((obs: any) => {
            return parseFloat(obs[seriesId]);
    ...
    it('should return an error for an invalid series code', () => {
      cy.request({
        method: 'GET',
        url: '/observations/INVALID_SERIES',
        failOnStatusCode: false
      }).then((response) => {
    ...
    it('should return an error for an invalid date range', () => {
      ...
      cy.request({
        ...
        // Depending on API, you might receive a 400 Bad Request.
    ...
    // Note: Triggering a 500 may not be possible unless the API has a known scenario.
    // You could stub the response or hit an endpoint known to cause an error.
    // For demonstration, we'll assume an endpoint `/observations/serverError` returns a 500.
    it('should handle a server error (500) scenario', () => {
      cy.request({
        method: 'GET',
        url: '/observations/serverError',
        failOnStatusCode: false
      }).then((response) => {
